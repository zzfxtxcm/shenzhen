...
(function($){
	...
	$.fn.commentWin=function(options){
		options = $.extend({},defaults,options);
		this.each(function(){
			var self = $(this);
			var commentBox = $(options.commentBox);
			...
			self.bind("click",function(event){
				...
				var boxHeight = commentBox.outerHeight();
				var boxWeight = commentBox.outerWidth();
				$(document).bind('click', func);
				event.stopPropagation();
				commentBox.css({'top':offset.top-boxHeight+13,'left':offset.left+55})
				if(commentBox.is(":hidden")){
					if((boxWeight+offset.left+55)>$(window).width()){
						commentBox.css({'left':offset.left-boxWeight-5}).show();
					}
	...
	$.fn.GTab= function(options){
		options = $.extend({},defaults,options);
 		return this.each(function(){
   			var self = $(this),
				oTag = self.find(options.tag).children(),
				oPanel = self.find(options.panel),
				len = oTag.length,
				cur = 0;
				oTag.eq(options.showNum-1).addClass('on')
				oPanel.eq(options.showNum-1).show().siblings(options.panel).hide();
			...
			/*×Ô¶¯²¥·Å
			if(options.auto){
				t = setInterval(auto,options.time);

				oPanel.bind(options.mouse,function(){
					clearInterval(t)
				}).bind('mouseout',function(){
					t = setInterval(auto,options.time);
				})

				oTag.bind(options.mouse,function(){
					clearInterval(t)
				}).bind('mouseout',function(){
					t = setInterval(auto,options.time);
				})
			...
			*/
			...
			function changeTag(){
				oTag.bind(options.mouse,function(){
				var index = oTag.index(this);
				oTag.eq(index).addClass('on').siblings().removeClass('on');
				oPanel.eq(index).show().siblings(options.panel).hide();
				})
			}
			...
			function auto(){	
				...
				oPanel.eq(cur).show().siblings(options.panel).hide();
			}
...
$(function(){
	$('.item-tab').GTab({'auto':'true'});
	$('.price-tab').GTab({'mouse':'click'});
})
